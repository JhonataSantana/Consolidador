...
class Consolidador:
    ...
    def __init__(
        ...
        self._files = self.getFiles(folder_path, file_types)
    ...
    def getData(self, file_path: str) -> DataFrame:
        ...
        if ".csv" in file_path:
            data = pd.read_csv(file_path, sep=self._sep, delimiter=self._delimiter, encoding="UTF-8", index_col=False)
    ...
    def consolidate(self) -> DataFrame:
        ...
        files: list[str] = self.applyFilters(self._files, self._filters)
